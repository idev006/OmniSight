<template>
  <div class="flex flex-col gap-5">
    ...
    <div class="flex items-start justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-base-content/40 mt-0.5">
          Face recognition attendance records &amp; reports
        </p>
      </div>
      <button
        v-if="activeTab === 'logs'"
        ...
        <template #cell-full_name="{ row }">
          <div class="flex items-center gap-2">
            ...
            <div
              ...
              :title="
                row.snapshot_url ? 'คลิกดูใบหน้าหลักฐาน' : 'ไม่มีภาพหลักฐาน'
              "
            >
              <div
              ...
              <div
                v-else
                class="bg-neutral text-neutral-content rounded-full w-8 h-8 flex items-center justify-center text-xs"
              >
    ...
    <template v-else-if="activeTab === 'daily'">
      ...
      <div class="flex gap-2 flex-wrap items-end">
        ...
        <div class="form-control">
          <label class="label py-1"
            ><span class="label-text text-xs">Department</span></label
          >
      ...
      <div
        v-if="dailyReport"
        class="stats bg-base-100 border border-base-300 shadow-sm w-full"
      >
        <div class="stat py-3">
        ...
        <div class="stat py-3">
          ...
          <div class="stat-desc">
            &gt; {{ dailyReport.late_threshold_minutes }} min after shift
          </div>
        </div>
        <div class="stat py-3">
      ...
      >
        <table class="table table-sm">
          ...
          <tbody>
            <tr
              v-for="r in dailyReport.records"
              :key="r.employee_id"
            ...
            >
              ...
              <td>
                <div class="flex flex-col text-xs">
                  ...
                  <span class="text-base-content/40"
                    >{{ r.shift_start }} – {{ r.shift_end }}</span
                  >
                </div>
              </td>
              ...
              <td class="text-center">
                <span
                  v-if="r.status === 'LATE'"
                  class="font-mono text-warning font-semibold text-sm"
                >
                  +{{ r.minutes_late }}
                </span>
                <span
                  v-else-if="r.status === 'ABSENT'"
        ...
        <div
          v-if="dailyReport.records.length === 0"
      ...
      <div
        ...
        class="py-8 text-center text-sm text-base-content/30"
      >
    ...
    <template v-else-if="activeTab === 'summary'">
      ...
      <div class="flex gap-2 flex-wrap items-end">
        <div class="form-control">
          <label class="label py-1"
            ><span class="label-text text-xs">Month</span></label
          >
          <input
      ...
      <div v-else-if="summary" class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div
          class="xl:col-span-2 bg-base-100 rounded-xl border border-base-300 p-5"
        >
          <div class="text-sm font-semibold mb-4 text-base-content/70">
            Daily Attendance — {{ summary.month }}
          </div>
          <div class="flex items-end gap-0.5 h-36" v-if="dayMax > 0">
            <div
              v-for="day in summary.by_day"
              :key="day.date"
            ...
            >
              ...
              <div
                ...
                :style="{
                  height:
                    day.count > 0
                      ? Math.max(4, (day.count / dayMax) * 128) + 'px'
                      : '2px',
                }"
              ></div>
              ...
              <div
                v-if="
                  Number(day.date.slice(8)) % 5 === 0 ||
                  Number(day.date.slice(8)) === 1
                "
              ...
              <!-- Tooltip -->
              <div
                class="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 hidden group-hover:flex flex-col items-center bg-base-content text-base-100 text-[10px] rounded px-1.5 py-1 whitespace-nowrap z-10 shadow"
              >
                {{ day.date.slice(8) }} {{ monthName }} — {{ day.count }} scans
                / {{ day.unique_employees }} ppl
                <div
                  class="absolute top-full border-4 border-transparent border-t-base-content"
                ></div>
              </div>
            </div>
          </div>
          <div
            v-else
            class="flex items-center justify-center h-36 text-base-content/25 text-sm"
          >
        ...
        <div class="bg-base-100 rounded-xl border border-base-300 p-5">
          ...
          <div
            v-if="summary.by_department.length === 0"
          ...
          <div v-else class="flex flex-col gap-2.5">
            <div
              v-for="dept in summary.by_department"
              :key="dept.dept_id"
              class="flex flex-col gap-1"
            >
              <div class="flex justify-between items-center text-xs">
                <span class="font-medium truncate">{{ dept.dept_name }}</span>
                <span class="text-base-content/40 font-mono shrink-0 ml-2">{{
                  dept.count
                }}</span>
              </div>
              <div
                class="w-full h-1.5 bg-base-300 rounded-full overflow-hidden"
              >
                <div
                  ...
                  :style="{
                    width:
                      Math.max(
                        4,
                        (dept.count / summary.total_records) * 100,
                      ).toFixed(1) + '%',
                  }"
                ></div>
              </div>
              <div class="text-[10px] text-base-content/30">
                {{ dept.unique_employees }} unique employees
              </div>
            </div>
          </div>
        </div>
  ...
  <dialog ref="snapshotModal" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box max-w-sm p-5" v-if="snapshotRow">
      <div class="flex items-center justify-between mb-3">
        <div>
          ...
          <div class="text-xs text-base-content/40 mt-0.5">
            {{ formatDate(snapshotRow.timestamp) }}
            {{ formatTime(snapshotRow.timestamp) }} ·
      ...
      <!-- Snapshot image -->
      <div
        class="rounded-xl overflow-hidden bg-base-300 aspect-square flex items-center justify-center"
      >
        <img
          v-if="snapshotBlobUrl"
          :src="snapshotBlobUrl"
      ...
      <!-- Metadata -->
      <div
        class="flex items-center justify-between mt-3 text-xs text-base-content/50"
      >
        <span class="font-mono">{{ snapshotRow.emp_code }}</span>
        <span
          class="badge badge-sm font-mono"
          :class="
            snapshotRow.confidence_score >= 0.85
              ? 'badge-success'
              : snapshotRow.confidence_score >= 0.72
                ? 'badge-warning'
                : 'badge-error'
          "
        >
...
function exportSummaryCSV() {
  ...
  const rows = summary.value.by_day.map((d) => [
    d.date,
    d.count,
    d.unique_employees,
  ]);
...
async function fetchDaily() {
  ...
  try {
    ...
    const { data } = await api.get("/api/v1/attendance/daily-report", {
      params,
    });
...
function exportDailyCSV() {
  if (!dailyReport.value?.records?.length) {
    toast.warning("No records to export");
    return;
  }
  const header = [
...
async function exportDailyPDF() {
  if (!dailyReport.value) {
    toast.warning("Load a daily report first");
    return;
  }
  ...
  try {
    ...
    if (dailyDept.value) params.dept_id = dailyDept.value;
    const response = await api.get("/api/v1/attendance/daily-report/pdf", {
      params,
      responseType: "blob",
    });
    const blob = new Blob([response.data], { type: "application/pdf" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    const filename = `attendance_${dailyDate.value || new Date().toISOString().slice(0, 10)}.pdf`;
    a.href = url;
    a.download = filename;
    a.click();
    URL.revokeObjectURL(url);
