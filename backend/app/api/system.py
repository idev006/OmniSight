...
from app.db.qdrant import qdrant as _qdrant
...
async def _postgres_info() -> dict:
    try:
        async with async_session_factory() as db:
            ...
            # DB size
            size_bytes = (await db.execute(text("SELECT pg_database_size(current_database())"))).scalar() or 0
            ...
            enrolled = (await db.execute(text("SELECT COUNT(DISTINCT employee_id) FROM face_templates"))).scalar()
